...
// Erklärung:
filter -> Filtert die Zahlen größer 10 und map verdoppelt alle zahlen die filter gefiltert hat und verdoppelt diese
Die verdoppelten zahlen werden in 'filterd' gespeichert
...
```js
// Dein Code hier:
let avgNoten = noten.reduce((acc, note) =>  { return acc += note/noten.length },0);

// Erklärung:
Der Akkumulator ist in diesem fall der acc 
Der Startwert ist 0, weil man ab dem zeit punkt halt startet, sonst würde das Ergebnis verfälscht werden
